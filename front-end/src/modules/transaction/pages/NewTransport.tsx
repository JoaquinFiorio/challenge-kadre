...
export const NewTransport = () => {
    ...
    const { newTransport, creating } = useTransport();
    ...
    const handleSubmit = async () => {
        const response: any = await newTransport(form);
        if (response?.success) {
            handleGoBack();
        }
    };
    ...
    return (
        <Padding>
            <Breadcrumb pageName={'Nuevo Transporte'} />
            <Card>
                <FieldEnvelope>
                    <Input
                        label="Camión"
                    ...
                    <Input
                        ...
                        type="datetime-local"
                        variant="underlined"
                        value={form.departure_date.toString()}
